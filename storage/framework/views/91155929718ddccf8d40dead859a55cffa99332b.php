<div x-data="{ open: false }" class="w-full">
    ...
    <div @click="open = true" class="group w-full bg-white px-4 py-4 sm:px-6 rounded-xl shadow-sm border border-gray-200 hover:shadow-md hover:border-green-500/50 hover:bg-yellow-500/10 transition-all duration-300 flex justify-between items-center cursor-pointer mb-2">
        ...
        <div class="flex items-center gap-4 sm:gap-6">
            ...
            <div class="flex flex-col justify-center">
                <h2 class="sm:text-3xl text-xl font-bold text-green-950 mb-1 group-hover:text-green-800 transition-colors"><?php echo e($evento->name); ?></h2>
                <h2 class="sm:text-3xl text-xl font-bold text-green-950 mb-1 group-hover:text-green-800 transition-colors"><?php echo e($evento->pueblo->name); ?></h2>
                <h3 class="sm:text-lg text-sm text-gray-600 font-medium flex items-center gap-2 leading-none">
                    <span><?php echo e($evento->dateIni); ?></span>
                    <i class="fa-solid fa-arrow-right text-yellow-400 flex items-center justify-center"></i>
                    <span><?php echo e($evento->dateFin); ?></span>
                </h3>
            </div>
        </div>
    ...
    <div x-show="open" style="display: none;" x-transition.opacity.duration.300ms class="fixed inset-0 bg-black/60 backdrop-blur-sm flex justify-center items-center z-50 p-4 sm:p-6">
        ...
        <div @click.away="open = false" x-transition.scale.origin.bottom.duration.300ms class="bg-green-950 p-6 sm:p-8 rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto relative flex flex-col scrollbar-hide">
            ...
            <!-- Cabecera -->
            <div class="mb-6">
                <h2 class="text-amber-50 text-3xl sm:text-4xl font-extrabold mb-3"><?php echo e($evento->name); ?></h2>
                <h3 class="text-amber-50 text-3xl sm:text-4xl font-extrabold mb-3"><?php echo e($evento->pueblo->name); ?></h3>
                <p class="text-gray-300 text-base sm:text-lg leading-relaxed"><?php echo e($evento->description ?? 'Descripción del pueblo no disponible en este momento.'); ?></p>
                ...
                <a href="<?php echo e(asset('storage/carteles/' . $evento->cartel)); ?>" target="_blank" class="inline-block">
            ...
            <div>
                ...
                <div class="rounded-xl overflow-hidden shadow-lg border border-green-800 bg-green-900">
                    <iframe
                        ...
                        loading="lazy"
                        allowfullscreen
                        src="https://www.google.com/maps?q=<?php echo e($evento->pueblo->latitude); ?>,<?php echo e($evento->pueblo->longitude); ?>&z=15&output=embed">
                    </iframe>
                </div>
            </div>
