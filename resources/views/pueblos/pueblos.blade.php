<div x-data="{ open: false }">
    ...
    <div x-show="open" x-transition class="fixed inset-0 bg-black/50 flex justify-center items-center z-50 overflow-auto">
        <div class="bg-green-950 p-6 rounded-xl shadow-xl w-full max-w-4xl relative mt-auto">
            ...
            <p class="text-amber-50 mb-4">{{ $pueblo->description ?? 'Descripción del pueblo aquí.' }}</p>
            <div x-data="{currentImage: 0,images: {{ $pueblo->image ? json_encode($pueblo->image) : '[]' }}}" class="relative mb-4">
                <img
                    :src="images.length > 0 ? images[currentImage] : '{{ asset('images/placeholder.png') }}'"
