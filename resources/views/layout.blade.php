...
<head>
    ...
    <!-- Alpine.js (Para interactividad sin jQuery) -->
    ...
    <script>
        tailwind.config = {
            theme: {
                extend: {
                    fontFamily: {
                        sans: ['Inter', 'sans-serif'], // Establecido como fuente por defecto
                    },
                    colors: {
                        // Paleta de la App para unificar colores si lo necesitas en el futuro
                        fiesta: {
                            green: '#14532d', // green-950
...
<body class="bg-green-950 flex flex-col min-h-screen text-amber-50 font-sans selection:bg-yellow-400 selection:text-green-950">
    ...
    <!-- Navbar con Alpine.js para mejor manejo del estado -->
    <nav x-data="{ mobileMenuOpen: false, profileDropdownOpen: false }" class="bg-green-950 sticky top-0 z-50 border-b border-yellow-400/50 shadow-md backdrop-blur-md bg-opacity-95">
            ...
            <div class="flex h-16 items-center justify-between">
                ...
                <div class="flex sm:hidden">
                    <button @click="mobileMenuOpen = !mobileMenuOpen" type="button" class="inline-flex items-center justify-center p-2 rounded-md text-yellow-400 hover:text-white hover:bg-green-800 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-yellow-400 transition-colors">
                        ...
                        <!-- Icono Cruz (cuando está abierto) -->
                ...
                <div class="flex items-center shrink-0">
                    <a href="{{ route('index') }}" class="group flex items-center gap-2">
                        <img class="h-10 w-auto group-hover:scale-105 transition-transform drop-shadow-md" src="{{ asset('images/logos/LOG_TEXT_AMARILLO.png') }}" alt="FiestaRioja Logo" />
                    </a>
                </div>

                <!-- Navegación Escritorio -->
                <div class="hidden sm:flex space-x-2 lg:space-x-4">
                    <a href="{{ route('index') }}" class="{{ request()->routeIs('index') ? 'bg-yellow-400 text-green-950 shadow-sm' : 'text-amber-50/90 hover:bg-green-800 hover:text-white' }} px-4 py-2 rounded-lg text-sm font-semibold transition-all">
                        Inicio
                    </a>
                    ...
                    <a href="{{ route('list') }}" class="{{ request()->routeIs('list') ? 'bg-yellow-400 text-green-950 shadow-sm' : 'text-amber-50/90 hover:bg-green-800 hover:text-white' }} px-4 py-2 rounded-lg text-sm font-semibold transition-all">
                        Pueblos
                    </a>
                </div>
                ...
                <div class="relative ml-3 shrink-0">
                    @auth
                        ...
                        <button @click="profileDropdownOpen = !profileDropdownOpen" @click.away="profileDropdownOpen = false" type="button" class="flex items-center text-sm rounded-full focus:outline-none focus:ring-2 focus:ring-yellow-400 focus:ring-offset-2 focus:ring-offset-green-950 transition-all">
                            ...
                            <div class="relative w-9 h-9 sm:w-10 sm:h-10 rounded-full border-2 border-yellow-400 overflow-hidden hover:scale-105 transition-transform">
                                @if (Auth::user()->avatar)
                                    <img class="w-full h-full object-cover" src="{{ asset('storage/' . Auth::user()->avatar) }}" alt="Avatar de usuario"/>
                                @else
                                    <img class="w-full h-full object-cover" src="{{ asset('images/default-profile.jpg') }}" alt="Avatar por defecto"/>
                                @endif
                            </div>
                        </button>
                        ...
                        <div x-show="profileDropdownOpen" 
                            ...
                            <!-- Cabecera del menú (nombre usuario) -->
                            <div class="px-4 py-3 border-b border-green-800/50">
                                <p class="text-sm font-medium text-green-300">Conectado como</p>
                                <p class="text-sm font-bold text-amber-50 truncate">{{ Auth::user()->name }}</p>
                            </div>

                            <a href="{{ route('perfil') }}" class="group flex items-center px-4 py-2.5 text-sm text-amber-50/90 hover:bg-green-800 hover:text-white transition-colors">
                    ...
                    @else
                        ...
                        <a href="{{ route('login') }}" class="inline-flex items-center gap-2 bg-yellow-400 text-green-950 hover:bg-yellow-300 hover:shadow-lg px-4 py-2 sm:px-5 sm:py-2.5 rounded-lg text-sm font-bold transition-all hover:-translate-y-0.5">
        ...
        <div x-show="mobileMenuOpen" 
            ...
            <div class="space-y-1 px-4 pb-4 pt-3">
                <a href="{{ route('index') }}" class="{{ request()->routeIs('index') ? 'bg-yellow-400 text-green-950' : 'text-amber-50 hover:bg-green-800' }} block rounded-md px-3 py-3 text-base font-bold transition-colors">
                ...
                <a href="{{ route('list') }}" class="{{ request()->routeIs('list') ? 'bg-yellow-400 text-green-950' : 'text-amber-50 hover:bg-green-800' }} block rounded-md px-3 py-3 text-base font-bold transition-colors">
    ...
    <!-- Contenedor Principal (Flex grow empuja el footer hacia abajo) -->
    ...
    <footer class="bg-green-950 border-t border-green-900 pt-12 pb-8 px-4 sm:px-6 lg:px-8 mt-auto">
        <div class="max-w-7xl mx-auto flex flex-col items-center">
            ...
            <div class="mb-8 opacity-80 hover:opacity-100 transition-opacity">
                <img class="h-12 w-auto grayscale" src="{{ asset('images/logos/LOG_TEXT_AMARILLO.png') }}" alt="FiestaRioja Logo" />
            </div>
            ...
            <nav class="flex flex-wrap justify-center gap-x-8 gap-y-4 mb-10 text-sm font-medium">
                ...
                <a href="#" class="text-green-300 hover:text-yellow-400 transition-colors">Política de Privacidad</a>
            ...
            <p class="text-xs text-green-500/70 text-center">
                &copy; {{ date('Y') }} FiestaRioja. Todos los derechos reservados.
            </p>
        </div>
    </footer>

    <!-- Botón Subir Arriba (Alpine.js) -->
