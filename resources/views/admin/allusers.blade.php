                    ...
                    <tbody>
                        @foreach ($users as $user)
                            <tr id="user-{{ $user->id }}" class="hover:bg-gray-50">
                                ...
                                <td class="px-4 py-2 text-center border" id="user-name-{{ $user->id }}">
                                    <span>{{ $user->name }}</span>
                                    <input type="text" value="{{ $user->name }}" class="hidden border rounded px-2 py-1 w-full" id="edit-name-{{ $user->id }}">
                                </td>
                                <td class="px-4 py-2 text-center border" id="user-email-{{ $user->id }}">
                                    <span>{{ $user->email }}</span>
                                    <input type="email" value="{{ $user->email }}" class="hidden border rounded px-2 py-1 w-full" id="edit-email-{{ $user->id }}">
                                </td>
                                <td class="px-4 py-2 text-center border" id="user-role-{{ $user->id }}">
                                    <span>{{ $user->role }}</span>
                                    <select name="role" class="hidden border rounded px-2 py-1 w-full" id="edit-role-{{ $user->id }}">
                                ...
                                <td class="px-4 py-2 text-center border">
                                    <div class="flex justify-center gap-4" id="icons-{{ $user->id }}">
                                        <button onclick="toggleEditFields({{ $user->id }})" class="text-blue-600 hover:text-blue-800 transition">
                                        ...
                                        <button onclick="confirmDelete({{ $user->id }})" class="text-red-600 hover:text-red-800 transition">
                                    ...
                                    <button class="hidden mt-2 px-3 py-1 bg-green-600 text-white rounded text-sm hover:bg-green-700"
                                        id="save-{{ $user->id }}" onclick="saveChanges({{ $user->id }})">
                                        Guardar
                                    </button>
                                    ...
                                    <form id="update-form-{{ $user->id }}" action="{{ route('users.update', $user->id) }}" method="POST" class="hidden">
                                        @csrf
                                        <input type="hidden" name="name" id="hidden-name-{{ $user->id }}" value="{{ $user->name }}">
                                        <input type="hidden" name="email" id="hidden-email-{{ $user->id }}" value="{{ $user->email }}">
                                        <input type="hidden" name="role" id="hidden-role-{{ $user->id }}" value="{{ $user->role }}">
                                    </form>
                                </td>
                            </tr>
                        @endforeach
